...
export class DashboardPage implements OnInit {
  ...
  get embedCode(){
    let showCreditsText = this.showCreditsText ? '&showCreditsText=1' : '';
    return `<iframe id="alicialabs_marketplace" width="${this.embedWidth}" height="${this.embedHeight}" style="border: none;" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe><script>const iframe = document.getElementById("alicialabs_marketplace");iframe.src = "https://conversation.alicialabs.com/marketplace/filter?trainerIds=${this.nav.activeOrganisationId}${showCreditsText}&ts=" + Date.now();</script>`
  }
